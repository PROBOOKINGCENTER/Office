...
class Airline_Tour extends Model
{
	public function __construct() {
		parent::__construct();
    }

    private $_objType = 'airline';
    private $_table = 'airline';
    private $_field = '*';
    private $_prefixField = 'air_';
    private $_uploadFolder = 'tour/airline/';
    ...
    public function convert($data){
        ...
        if( !empty($data['url_img']) ){
            $data['image_url'] = UPLOADS.$this->_uploadFolder.$data['url_img'];
        }
    ...
    public function deleteImage($id, $filename)
    {
        ...
        if( file_exists($source) ){
            unlink($source);
            $this->update($id, array('air_url_img'=>''));
        }
    }
}
