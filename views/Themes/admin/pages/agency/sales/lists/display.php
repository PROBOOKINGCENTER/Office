...
<script type="text/javascript">
	...
	$('body').delegate('[data-table-action=star]', 'click', function(event) {
		
		var $box = $(this).closest('tr'),
			is = $box.hasClass('has-star');

		$.ajax({
			url: app.getUri('agency/update/sales/' + $box.attr('item-id') ) ,
			type: 'POST',
			dataType: 'json',
			data: { name: 'agen_star', value: !is ? 1: 0},
		})
		.done(function(res) {
			$box.toggleClass('has-star', !is);

			if( is ){
				$box.find('.js-star').html( '<i class="icon-star-o"></i>' );
			}
			else{
				$box.find('.js-star').html( '<i class="icon-star"></i>' );
			}

			Event.log({ text: res.message, auto: true, delay: 1000 })
		})
		.fail(function() {
			// console.log("error");
		})
	...
	$(function () {
		...
		$('body').delegate('#auto_password', 'change', function () {
			...
			var is = $(this).prop('checked'),
				$fieldset = $('.form-agency-sales-right #agen_password_fieldset');
		...
		var password;
		$('body').delegate('.form-agency-sales-wrap', 'submit', function (e) {
			e.preventDefault();
