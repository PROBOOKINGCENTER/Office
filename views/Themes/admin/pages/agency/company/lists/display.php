...
<script type="text/javascript">
	...
	$(function () {
		...
		$('body').delegate('[data-table-action=guarantee]', 'click', function(event) {
		
			var $box = $(this).closest('tr'),
				is = $box.hasClass('has-guarantee');


			$.ajax({
				url: app.getUri('agency/update/company/' + $box.attr('item-id') ) ,
				type: 'POST',
				dataType: 'json',
				data: { name: 'agen_com_guarantee', value: !is ? 1: 0},
			})
			.done(function(res) {
				// console.log("success");

				$box.toggleClass('has-guarantee', !is);
				if( is ){
					$box.find('.js-guarantee').html( '<i class="icon-thumbs-o-up"></i>' );
				}
				else{
					$box.find('.js-guarantee').html( '<i class="icon-thumbs-up"></i>' );
				}
