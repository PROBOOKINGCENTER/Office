			...
			<div class="tb-header-inner">
				...
				<nav class="tb-filter" role="filter">
					...
					<ul class="clearfix" style="margin-top: 2px;"><?php
						...
						if( !empty($this->bookingStatusList) ){

							echo '<li class="filter-item rfloat">';
							foreach ($this->bookingStatusList as $key => $value) {

								if( isset($value['disabled']) || empty($value['is_payment']) ) continue;
								$name = isset($value['desc']) ? $value['desc']: $value['name'];
								echo '<label class="checkbox mrs"><input type="checkbox" name="book_status" value="'.$value['id'].'" data-action="multichecked"><span class="fwb">'.$name.'</span></label>';
							}
							echo '</li>';
