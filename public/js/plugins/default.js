...
(function( $, window, document, undefined ) {
    ...
    var Dropdown2 = {
		init: function( options, elem ) {
			...
			self.$elem = $(elem);
			self.options = $.extend( {}, $.fn.dropdown2.options, options );

			if( !self.options.select) return false;

			if( self.options.select.length==0 )return false;
		...
		setMenu: function () {
			...
			self.$model = $('<div/>', {class: 'uiContextualLayer uiContextualPositioner'});
			self.$layer = $('<div/>', {class: 'uiContextualLayerPositioner uiLayer'});

			self.$layer.append( self.$model.append( $('<div/>', {class: 'uiTypeaheadView'}) ).html( self.$menu ) );

			if( self.options.container=='parent' ){
				self.$elem.parent().append( self.$layer );
			}
			else{
				$( self.options.container ).append( self.$layer );
			}
		...
		offset: function () {
			...
			if( self.options.container=='parent' ){
			...
			else if( self.options.container=='body' ){
				var $parent = $( self.options.container );

				offset = self.$elem.offset();

				if( (offset.top + self.$elem.outerHeight() + self.$model.outerHeight()) > $parent.outerHeight() ){
				...
				var $parent = $( self.options.container );
				var parentH = $parent.children().outerHeight()>$parent.outerHeight() ? $parent.children().outerHeight(): $parent.outerHeight();
				offset = self.$elem.position();

				if( (offset.top + self.$elem.outerHeight() + self.$model.outerHeight()) > parentH ){
					offset.top -= self.$elem.outerHeight();
				}
				else{
					offset.top += self.$elem.outerHeight();
			...
			if( self.options.axisX=='left' || self.options.axisX=='right' ){
				...
				if( self.options.axisX=='right' ){
					self.$model.addClass('uiToggleFlyoutRight');

					offset.left += self.$elem.outerWidth();
				}
			}
			else if( self.options.container!='parent' ){

				if( (offset.left + self.$elem.outerWidth() + self.$model.outerWidth()) > $parent.width() ){
					self.$model.addClass('uiToggleFlyoutRight');
					offset.left += self.$elem.outerWidth();
				}
				else{
					self.$model.remnoveClass('uiToggleFlyoutRight');
				}
			}


			self.$layer.css( offset );

			// var offset = self.$layer


			/*if( self.options.axisX ){
				uiToggleFlyoutRight
			}*/
		},
		close: function() {
	...
	$.fn.dropdown2 = function( options ) {
		return this.each(function() {
			var obj = Object.create( Dropdown2 );
			obj.init( options, this );
			$.data( this, 'dropdown2', obj );
		});
	};
	$.fn.dropdown2.options = {
		// change: function(){},
        container: 'body',
	...
	var Tooltip = {
        ...
        get: function(){
            ...
            if( self.options.pointer ){
            	self.$layer.addClass('pointer');
            	offset.top += 12;
            }
            else{
            	self.$layer.removeClass('pointer');
            }
	...
	var CountdownDate = {
		init: function ( settings, elem  ) {
			...
			if( !self.data.date ) return;
			self.date = new Date(self.data.date);
			self.delay = self.data.delay || 10000;
		...
		run: function (length) {
			...
			var countDownDate = self.date.getTime();
			setTimeout(function() {
				...
				self.$elem.text( $.trim( txt ) );

				if (distance < 0) {
					self.$elem.addClass('is_expired').text( 'EXPIRED' );
				}
				else{
					self.run();
				}
	...
	$.fn.countdownDate = function( options ) {
		return this.each(function() {
			var $this = Object.create( CountdownDate );
			$this.init( options, this );
			$.data( this, 'countdownDate', $this );
		});
	};
	...
	var Dock = {
		init: function ( settings, elem  ) {
			...
			if( $('#alertMessages').length==0 ){
				self.$model = $('<div/>', {class:"alertMessages", id:"alertMessages"});
				$('body').append(self.$model);
			}
			else{
				self.$model = $('#alertMessages');
			} 


			self.$content = $('<div>', {class: 'inner'});
			self.$elem.addClass('message').html( self.$content );
			var li = 0;

			if( self.options.key ){
				$i = self.$model.find('[data-alert-id='+self.options.key+']');

				self.$elem.attr('data-alert-id', self.options.key);

				if( $i.length==1 ){
					li == 1;
					self.$elem = $i;
					self.$content = self.$elem.find('.inner');

					self.$elem.animate({opacity:.5}, 100).delay(300).animate({opacity:1}, 100);
					// $item.addClass('blinker');

					clearTimeout( self.$elem.data('timeout') );
				}
			}


			if( self.options.bg ){
				self.$elem.addClass( self.options.bg );
			}

			self.$content.empty().append( $('<div>', {class: 'close'}).html( '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" viewBox="0 0 348.333 348.334" xml:space="preserve"><g><path d="M336.559,68.611L231.016,174.165l105.543,105.549c15.699,15.705,15.699,41.145,0,56.85   c-7.844,7.844-18.128,11.769-28.407,11.769c-10.296,0-20.581-3.919-28.419-11.769L174.167,231.003L68.609,336.563   c-7.843,7.844-18.128,11.769-28.416,11.769c-10.285,0-20.563-3.919-28.413-11.769c-15.699-15.698-15.699-41.139,0-56.85   l105.54-105.549L11.774,68.611c-15.699-15.699-15.699-41.145,0-56.844c15.696-15.687,41.127-15.687,56.829,0l105.563,105.554 L279.721,11.767c15.705-15.687,41.139-15.687,56.832,0C352.258,27.466,352.258,52.912,336.559,68.611z"/></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g></svg>' ), $('<div>', {class: 'text'}).html( self.options.text ) );
			self.$model.append( self.$elem );
			...
			self.$elem.mouseenter(function() {
				...
				if( self.options.auto ){
					...
					// self.alertHide( $item, options.delay );
				}
			});*/
		...
		hide: function ( $elem ) {
			...
			setTimeout(function () {
				...
				if(self.$model.children().length==0){
					self.$model.remove();
				}
			}, 300);
		},
