...
(function( $, window, document, undefined ) {
	...
	var List = {
		...
		resize: function () {
			...
			self.$tablelists.find('table tr:not(.header):first>td').each(function ( i ) {
		...
		Events: function () {
			...
			self.$elem.find('.js-refresh').click(function (e) {
				...
				if( self.settings.is_load_more ){
					self.$elem.closest('[role=main]').scrollTop(0);
			...
			if( self.$header ){
				$.each( self.$header.find('[data-action=checked]'), function() {
					if( $(this).attr('name') ){
			...
			if( self.$header ){
				...
				self.$header.find('[data-action=checked]').change(function () {
			...
			if( self.settings.is_load_more ){
				self.$footeralert = self.$elem.find('.listpage2-table-footer-alert');
				self.$scroll = self.$elem.closest('[role=main]');

				self.$scroll.scroll(function() {
					
					var h = self.$tablelists.outerHeight() - $(this).outerHeight() + (self.$header.outerHeight() + self.$tabletitle.outerHeight());
					if( h - $(this).scrollTop() <= 200 && !self.$elem.hasClass('has-loading') && !self.has_loading && self.data.options.more ){
						self.loadMore();
					}
				});

				self.$elem.find('[role-scrollTop]').click(function() {
					self.$scroll.animate({scrollTop: 0 }, 'fast' );
				});
			}
		},
			...
			self.is_loading = setTimeout(function () {
				self.fetch().done(function( results ) {
					...
					if( self.data.options.sort ){
						...
						var sort = $('[data-sort-val='+ self.data.options.sort +']'),
							dir = self.data.options.dir.toLowerCase();
						if( sort.length==1 ){
	    ...
	    loadMore: function () {
	    	...
	    	self.$footeralert.removeClass('hidden_elem');
	    	...
	    	.done(function( res ) {
	    		...
	    		self.data.options = $.extend( {}, self.data.options, res.settings.options );
	    		self.$tablelists.find('tbody').append( $table.find('tr') );
	    		Event.plugins( self.$tablelists );
	    	})
	    	.fail(function() {
	    		// console.log("error");
	    	...
	    	.always(function() {
	    		self.has_loading = false;
	    		self.$footeralert.addClass('hidden_elem');
